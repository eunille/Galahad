...
const MemberInformation: React.FC<MemberInformationProps> = ({
  ...
  const validateField = (fieldName: string, value: string) => {
    ...
    if (!value.trim()) {
      ...
      if (
        ...
        error = `${fieldName.replace(/_/g, " ")} must be a valid phone number (09XXXXXXXXX).`;
      }
    }
    return error;
  };
  ...
  const validateForm = () => {
    ...
    ["first_name", "last_name", "contact", "emergency_contact"].forEach((key) => {
      const error = validateField(key, member[key as keyof typeof member] || "");
      ...
      <div>
        ...
        <span
          className={`block p-2 border rounded-md ${
            checkEligibility(selectedMemberData.membership, selectedMemberData.purchased_at) === "Active"
              ? "text-green-600"
              : "text-red-600"
          }`}
        >
          {checkEligibility(selectedMemberData.membership, selectedMemberData.purchased_at)}
        </span>
        </div>
      <div>
        <label className="block text-sm font-medium text-gray-700">
